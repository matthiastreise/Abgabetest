...
import type { Film, FilmArt, Studio } from './film';
...
export const filmSchema = new Schema<FilmDocument, Model<FilmDocument>>(
    {
        ...
        art: { type: String, enum: ['DVD', 'BlueRay', 'Download', 'VHS'] },
        studio: {
            type: String,
            required: true,
            enum: [
        ...
        lieferbar: Boolean,
        datum: Date,
        isan: { type: String, required: true, unique: true, immutable: true },
        regisseur: String,
        genre: { type: [String], sparse: true },
        // "anything goes"
        darsteller: [{}],
    },
    {
