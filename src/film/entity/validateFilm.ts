 ...
 * Funktion {@linkcode validateFilm} sowie notwendigen Konstanten.
...
export const validateFilm = (film: Film) => {
    const validate = ajv.compile<Film>(jsonSchema);
    validate(film);
    // nullish coalescing
    const errors = validate.errors ?? [];
    logger.debug('validateFilm: errors=%o', errors);
    ...
    errors.forEach((err) => {
        ...
        switch (instancePath) {
            ...
            case '/art':
                errorMsg.art =
                    'Die Art eines Filmes muss VHS, BluRay, Download oder DVD sein.';
                break;
            case '/studio':
                errorMsg.studio =
            ...
            case '/datum':
                ...
                break;
            case '/isan':
                errorMsg.isan = 'Die ISAN-Nummer ist nicht korrekt.';
                break;
            case '/regisseur':
                errorMsg.regisseur = 'Der Regisseur ist nicht korrekt.';
                break;
        }
    });

    logger.debug('validateFilm: errorMsg=%o', errorMsg);
