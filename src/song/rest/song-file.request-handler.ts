...
export class SongFileRequestHandler {
    private readonly service = new SongFileService();

    upload(req: Request, res: Response) {
        const { id } = req.params;
        logger.debug(`SongFileRequestHandler.uploadBinary(): id=${id}`);
        ...
        req.on('data', (chunk: Uint8Array) => {
            const { length } = chunk;
            logger.debug(
                `SongFileRequestHandler.uploadBinary(): data ${length}`,
            );
            data.push(chunk);
            totalBytesInBuffer += length;
        })
            .on('aborted', () =>
                logger.debug('SongFileRequestHandler.uploadBinary(): aborted'),
            )
            .on('end', () => {
                logger.debug(
                    `SongFileRequestHandler.uploadBinary(): end ${totalBytesInBuffer}`,
                );
    ...
    async download(req: Request, res: Response) {
        const { id } = req.params;
        logger.debug(`SongFileRequestHandler.downloadBinary(): ${id}`);
        if ((id as string | undefined) === undefined) {
            res.status(HttpStatus.BAD_REQUEST).send('Keine Song-Id');
            return;
        }

        const findResult = await this.service.find(id);
        if (
            findResult instanceof SongFileServiceError ||
            findResult instanceof SongNotExists
        ) {
    ...
    private handleDownloadError(
        err: SongNotExists | DownloadError,
        res: Response,
    ) {
        if (err instanceof SongNotExists) {
            ...
            logger.debug(
                `SongFileRequestHandler.handleDownloadError(): msg=${msg}`,
            );
