...
// T e s t d a t e n
// -----------------------------------------------------------------------------
const titelVorhanden = ['a', 't', 'e'];
const titelNichtVorhanden = ['xx', 'yy'];
const genreVorhanden = ['comedy', 'abenteuer'];
const genreNichtVorhanden = ['thriller', 'romantic'];
...
describe('GET /api/filme', () => {
    ...
    each(genreVorhanden).test(
        ...
        async (schlagwort) => {
            ...
            // Jedes Film hat im Array der Schlagwoerter "comedy"
            body.map((film: Film) => film.genre).forEach((s: Array<string>) =>
                expect(s).to.include(schlagwort.toUpperCase()),
            );
        },
    );

    each(genreNichtVorhanden).test(
